...
from server_app.push_notification_tools import account_with_token
...
def link_token():
    ...
    result = account_with_token(account=user.username, platform=platform, token=token, operator_type=1)
    return result, 200


@pn_blueprint.route('/unlink_token', methods=['POST'])
@login_required
def unlink_token():
    """
    @api {post} /v1/push/unlink_token 取消关联token至当前帐号
    @apiVersion 1.0.0
    @apiName unlink_token
    @apiGroup Push Notification
    @apiParam {String}  token       (必须)    推送通知的token
    @apiParam {String}  platform    (必须)    平台(android/ios)
